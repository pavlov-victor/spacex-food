...
Показывать старую ссылку во время обработки можно, но с пометкой устаревшей версии (`isStale`). В списке меню подключена кнопка **PDF / Print**: превью всех страниц, Download PDF, Print, Open PDF и ручная генерация/повтор. При ограничениях встроенного просмотрщика используйте Open PDF и печать средствами браузера. File input для импорта PDF требует отдельного подключения.
