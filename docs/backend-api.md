...
- `useMenuWorkflows().generateAll(menuId)` → `{queued, skipped}`. Кнопка Generate all cards у меню: генерирует карточки для позиций без сохранённого изображения, пропускает готовые и активные. Задачи ставятся на сервере с интервалом старта 3 секунды; закрытие страницы не прерывает очередь. Повторное нажатие не дублирует активные задачи. Не публикует результаты автоматически.
- `useMenuPublishing(menuId).approveAll()` → `{approved, pending, failed}`. Кнопка Approve all generated cards применяет английский текст всех готовых непринятых карточек без обязательного открытия каждой. Активные генерации пропускаются, ошибочные карточки перечисляются по имени. Цена и подтверждённый состав не меняются. После завершения оставшихся генераций кнопку можно нажать снова, затем Publish menu.
